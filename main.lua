...
local userName = options.Name
local customMessage = options.Message
local autoTeleport = options.Teleport ~= false
...
local function finishLoading()
    ...
    vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
        if autoTeleport and (not teleportedServers) and (not shared.VapeIndependent) then
    ...
    if not shared.vapereload and not closetMode then 
        ...
        if vape.Categories.Main.Options['GUI bind indicator'].Enabled then
            local greeting = (userName and userName ~= "") and ("wsg " .. userName .. ", ") or ""
            local message
            
            if customMessage and customMessage ~= "" then
                message = customMessage
            else
                message = vape.VapeButton and 'Press the button in the top right to open GUI' or 'Press '..table.concat(vape.Keybind, ' + '):upper()..' to open GUI'
            end
            
            vape:CreateNotification('Finished Loading', greeting .. message, 5)
        end
    end
end
